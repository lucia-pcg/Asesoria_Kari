...
// Comentario
	...
	void printIMU() {
		...
		if (pros::millis() - last_print > 150) {
			...
			int yaw = (int)(IMU_SIGN * imu.get_yaw());
			master.print(0, 0, "Yaw:%4d      ", yaw);
		}
	}
	...
	void printMotors() {
		...
		if (pros::millis() - last_print > 150) {
			...
			int l_rpm = (int)left_mg.get_actual_velocity();
			int r_rpm = (int)right_mg.get_actual_velocity();

			master.print(0, 0, "L:%4d R:%4d  ", l_rpm, r_rpm);
		}
	}
